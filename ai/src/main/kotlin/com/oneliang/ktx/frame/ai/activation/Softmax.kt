...
fun softmax(xArray: Array<Float>, weightArray: Array<Array<Float>>, negative: Boolean = false): Array<Float> {
    val totalValueAtomicMap = AtomicMap<String, FloatWrapper>()
    val valueMatrix = xArray.multiply(weightArray, true, transform = {
        val value = exp(it)
        totalValueAtomicMap.operate(KEY_TOTAL_VALUE, create = { FloatWrapper(value) },
            update = { old ->
                FloatWrapper(old.value + value)
//            totalValue += value
            })
        value
    })
    //update value matrix to probability
    for (index in valueMatrix.indices) {
        valueMatrix[index] = valueMatrix[index] / totalValueAtomicMap[KEY_TOTAL_VALUE]!!.value * (if (negative) -1.0f else 1.0f)
    }
    return valueMatrix
}